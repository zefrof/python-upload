...
index = 0
for f in video_files:
	subprocess.run('ffmpeg  -i ' + f + ' -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:-1:-1,setsar=1" -c:v mpeg4 -q:v 4 -c:a libmp3lame -q:a 4 /home/zefrof/Documents/youtube/fixed/' + str(index)  + ".mp4", shell=True)
	index += 1

video_files = glob.glob("/home/zefrof/Documents/youtube/fixed/*.mp4")
...
print('youtube-upload --title="Best Of Idiots In Cars ' + now + '" --description="The best videos of Idiots In Cars from ' + now + '. Like, comment, and subscribe!" --category="Autos & Vehicles" --tags="idiots in cars, best of idiots in cars, idiots in cars highlights, idiots in cars top, idiots in cars reddit, idiots in cars compilation, idiots in cars reaction, idiots in cars police, idiots in cars clips, idiots in cars driving fails" --default-language="en" --default-audio-language="en" --client-secrets="/home/zefrof/Documents/youtube/client_secrets.json" --credentials-file="/home/zefrof/Documents/youtube/my_credentials.json" /home/zefrof/Documents/youtube/output.mkv')
...
subprocess.run("rm /home/zefrof/Documents/youtube/fixed/*.mp4", shell=True)
